...
- Model ids come from `GEMINI_TUTOR_MODEL` and `GEMINI_CLASSIFIER_MODEL` with **hardcoded literal fallbacks** (`gemini-3.6-flash`, `gemini-3.6-flash`) in the route handler. Audit patch 4c forbids exactly this, because a stale fallback survives a model upgrade silently. Open defect.
